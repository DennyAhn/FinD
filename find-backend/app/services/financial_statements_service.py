...
async def fetch_financial_statements_view(
    ...
    if sub_tab == "income":
        return await _build_income_statement_view(ticker, db, client, sub_tabs, period, year_range)
    elif sub_tab == "balance":
        return await _build_balance_sheet_view(ticker, db, client, sub_tabs, period, year_range)
    elif sub_tab == "cash_flow":
        return await _build_cash_flow_view(ticker, db, client, sub_tabs, period, year_range)
    else:
        return await _build_income_statement_view(ticker, db, client, sub_tabs, period, year_range)


async def _build_income_statement_view(
    ticker: str, db: Session, client: httpx.AsyncClient, sub_tabs: List[Dict], period: str, year_range: int = 3
) -> Dict[str, Any]:
    ...
    if period == "quarter":
        ...
        sorted_statements = sorted_statements[-quarters_to_keep:] if len(sorted_statements) > quarters_to_keep else sorted_statements
        ...
        if prev_revenue > 0:
            ...
            if period == "annual":
                ...
                if len(sorted_statements) >= 5:
                    same_quarter_last_year = sorted_statements[-5] if len(sorted_statements) >= 5 else None
                    if same_quarter_last_year:
                        prev_year_revenue = same_quarter_last_year.get("revenue") or 0
                        if prev_year_revenue > 0:
                            revenue_yoy = ((latest_revenue - prev_year_revenue) / prev_year_revenue) * 100
            ...
            if period == "annual":
                ...
                if len(sorted_statements) >= 5:
                    ...
                    if same_quarter_last_year:
                        ...
                        if prev_year_net > 0:
                            net_income_yoy = ((latest_net - prev_year_net) / prev_year_net) * 100
    ...
    if revenue_yoy is not None:
        highlights.append(
            f"최근 {period_label} 매출은 전년 동기 대비 {revenue_yoy:+.1f}% {'증가' if revenue_yoy > 0 else '감소'}했습니다."
        )
    if revenue_qoq is not None:
        highlights.append(
            f"전 분기 대비 매출은 {revenue_qoq:+.1f}% {'증가' if revenue_qoq > 0 else '감소'}했습니다."
        )
    
    if net_income_yoy is not None:
        highlights.append(
            f"순이익은 전년 동기 대비 {net_income_yoy:+.1f}% {'증가' if net_income_yoy > 0 else '감소'}했습니다."
        )
    if net_income_qoq is not None:
        highlights.append(
            f"전 분기 대비 순이익은 {net_income_qoq:+.1f}% {'증가' if net_income_qoq > 0 else '감소'}했습니다."
        )
    ...
    for stmt in sorted_statements:
        ...
        # Format period label
        if period == "annual":
            period_label = str(stmt.get("report_year") or stmt.get("report_date", "")[:4])
        else:
            ...
            if report_date:
                ...
                if "Q" in period_str.upper():
                    quarter = period_str.upper().split("Q")[-1].strip()[:1]
                elif month:
                ...
                period_label = str(stmt.get("report_year") or "")
    ...
    for idx, stmt in enumerate(sorted_statements):
        if period == "annual":
            ...
            if report_date:
                ...
                period_label = str(stmt.get("report_year") or "")
        period_to_stmt[period_label] = idx

    # Build table rows with professional terms
    table_rows.append({
        "id": "revenue",
        "label": "매출액 (Revenue)",
        **{p: (sorted_statements[period_to_stmt[p]].get("revenue") or 0) if p in period_to_stmt else 0 for p in periods},
    })
    table_rows.append({
        "id": "gross_profit",
        "label": "매출총이익 (Gross Profit)",
        **{p: (sorted_statements[period_to_stmt[p]].get("gross_profit") or 0) if p in period_to_stmt else 0 for p in periods},
    })
    table_rows.append({
        "id": "operating_income",
        "label": "영업이익 (Operating Income)",
        **{p: (sorted_statements[period_to_stmt[p]].get("operating_income") or 0) if p in period_to_stmt else 0 for p in periods},
    })
    table_rows.append({
        "id": "net_income",
        "label": "당기순이익 (Net Income)",
        **{p: (sorted_statements[period_to_stmt[p]].get("net_income") or 0) if p in period_to_stmt else 0 for p in periods},
    })
...
async def _build_balance_sheet_view(
    ticker: str, db: Session, client: httpx.AsyncClient, sub_tabs: List[Dict], period: str, year_range: int = 3
) -> Dict[str, Any]:
...
async def _build_cash_flow_view(
    ticker: str, db: Session, client: httpx.AsyncClient, sub_tabs: List[Dict], period: str, year_range: int = 3
) -> Dict[str, Any]:
    # Placeholder - 실제 구현 필요
    return {
